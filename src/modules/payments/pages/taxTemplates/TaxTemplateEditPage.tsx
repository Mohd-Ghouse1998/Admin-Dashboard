...
import { getTaxTemplate, updateTaxTemplate, TaxTemplate } from '@/services/api/taxTemplatesApi';
...
const TaxTemplateEditPage = () => {
  ...
  // Fetch tax template data on component mount
  useEffect(() => {
    if (id) {
  ...
  const fetchTaxTemplate = async (templateId: number) => {
    ...
    try {
      ...
      toast({
        title: 'Error',
        description: 'Failed to load tax template. Please try again.',
        variant: 'destructive',
      });
  ...
  const handleSubmit = async (values: TaxTemplateFormValues) => {
    ...
    try {
      await updateTaxTemplate(parseInt(id), values);
      toast({
        title: 'Success',
        description: 'Tax template updated successfully',
        variant: 'success',
      });
      navigate(`/payment/tax-templates/${id}`);
    } catch (error) {
      console.error('Failed to update tax template:', error);
      toast({
        title: 'Error',
        description: 'Failed to update tax template. Please try again.',
        variant: 'destructive',
      });
    } finally {
    ...
    return (
      <PageLayout
        ...
        breadcrumbs={[
          ...
          { label: 'Edit', url: `/payment/tax-templates/${id}/edit` }
        ]}
        actions={
  ...
  return (
    <PageLayout
      title="Edit Tax Template"
      description="Update tax template information"
      breadcrumbs={[
        { label: 'Payment & Billing', url: '/payment' },
        { label: 'Tax Templates', url: '/payment/tax-templates' },
        { label: form.getValues('name'), url: `/payment/tax-templates/${id}` },
        { label: 'Edit', url: `/payment/tax-templates/${id}/edit` }
      ]}
      actions={
        <Button variant="outline" onClick={handleBack}>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <Card className="lg:col-span-2">
              <CardHeader>
                ...
                <CardDescription>Update the basic information about the tax template</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <FormField
...
export default TaxTemplateEditPage;
