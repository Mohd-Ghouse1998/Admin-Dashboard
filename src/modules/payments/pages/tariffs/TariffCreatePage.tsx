...
import { createTariff, TimeRestriction, UserRestriction } from '@/services/api/tariffsApi';
...
const TariffCreatePage = () => {
  ...
  const [timeRestrictions, setTimeRestrictions] = useState<
    Array<{ day_of_week: string; start_time: string; end_time: string; multiplier: number }>
  >([]);
  const [userRestrictions, setUserRestrictions] = useState<
    Array<{ user_type: 'regular' | 'premium' | 'business'; multiplier: number }>
  >([]);
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const response = await createTariff(values as any);
      toast({
        title: 'Success',
        description: 'Tariff created successfully',
        variant: 'success',
      });
      navigate(`/payment/tariffs/${response.data.id}`);
    } catch (error) {
      console.error('Error creating tariff:', error);
      toast({
        title: 'Error',
        description: 'Failed to create tariff. Please try again.',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    <PageLayout
      title="Create Tariff"
      description="Define a new tariff plan for charging services"
      backButton
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Enter the basic details of the tariff plan
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Set the pricing structure for this tariff
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define different pricing multipliers for specific time periods
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Define different pricing multipliers for specific user types
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <Button
              type="button"
              variant="outline"
              onClick={() => navigate('/payment/tariffs')}
            >
              Cancel
            </Button>
