...
const promotionService = {
  createPromotion: async (accessToken: string, data: any) => {
...
// Form schema for promotion creation
...
const PromotionCreatePage = () => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      discount_value: 0,
      start_date: new Date(),
      end_date: new Date(new Date().setMonth(new Date().getMonth() + 1)), // Default to 1 month from now
      active: true,
    },
  });
  ...
  const createPromotionMutation = useMutation({
    mutationFn: (data: FormValues) => {
      ...
      return promotionService.createPromotion(accessToken, payload);
    },
    onSuccess: () => {
      toast({
        title: 'Success',
  ...
  return (
    ...
    >
      <Helmet>
        <title>Create Promotion | Admin Dashboard</title>
      </Helmet>
      
      <Card>
        <CardHeader>
          <CardTitle>Create Promotion</CardTitle>
          <CardDescription>Create a new promotional discount code for users</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="flex justify-end gap-4">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => navigate('/users/promotions')}
