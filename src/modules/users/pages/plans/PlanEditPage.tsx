...
const PlanEditPage = () => {
  ...
  useEffect(() => {
    if (plan) {
      // Format features as a string for the textarea
      const featuresString = plan.features ? plan.features.join('\n') : '';
  ...
  const updatePlanMutation = useMutation({
    mutationFn: (data: FormValues) => {
      if (!id) throw new Error('No plan ID available');
      ...
      return planService.updatePlan(accessToken, id, payload);
    },
    onSuccess: () => {
      toast({
        title: "Success",
  ...
  if (isLoading) {
    return (
      <PageLayout title="Edit Plan" description="Loading plan information...">
  ...
  return (
    <PageLayout
      ...
      breadcrumbs={[
        ...
        { label: plan?.name || 'Edit Plan', url: `/users/plans/${id}` },
        { label: 'Edit' }
      ]}
    >
      <Helmet>
        <title>Edit Plan | Admin Dashboard</title>
      </Helmet>
      
      <Card>
        <CardHeader>
          <CardTitle>Edit Plan</CardTitle>
          <CardDescription>Update the details of this subscription plan</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="flex justify-end gap-4">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => navigate(`/users/plans/${id}`)}
