...
const CPOCommandDetailPage: React.FC = () => {
  ...
  const { data: command, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['ocpi', 'cpo', 'command', id],
    queryFn: async () => {
      try {
        if (!id) throw new Error('Command ID is required');
        const response = await OCPIApiService.cpo.commands.getById(id);
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  {command.result ? (
                    ...
                    <p className="text-muted-foreground">No result information available.</p>
                  )}
                </div>
              </CardContent>
...
export default CPOCommandDetailPage;
