...
const EMSPCommandHistoryPage: React.FC = () => {
  const { toast } = useToast();
  const { syncRoleWithBackend, role } = useOCPIRole();
  const navigate = useNavigate();
  
  // Filters state
  const [commandTypeFilter, setCommandTypeFilter] = useState<string>('all_types');
  const [statusFilter, setStatusFilter] = useState<string>('all_statuses');
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [dateRangeFilter, setDateRangeFilter] = useState<string>('all_dates');
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    queryKey: ['ocpi', 'emsp', 'commands', commandTypeFilter, statusFilter, searchTerm, dateRangeFilter],
    queryFn: async () => {
      try {
        const response = await OCPIApiService.emsp.commands.getAll(getQueryParams());
  ...
  const handleViewCommand = (commandId: string) => {
    navigate(`/ocpi/emsp/commands/${commandId}`);
  };
  
  // Handle navigation to command center
  ...
  return (
    <PageLayout title="Command History" description="View and manage commands sent to charging points">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          <div>
            <CardTitle>EMSP Command History</CardTitle>
            <CardDescription>View and track commands sent to charging points</CardDescription>
          </div>
          <div className="flex space-x-2">
            <Button onClick={handleNavigateToCommandCenter}>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">
              ...
              <div className="space-y-4 mt-4">
                ...
                <Button onClick={handleNavigateToCommandCenter} className="mx-auto mt-2">
                  <Send className="mr-2 h-4 w-4" />
                  Send New Command
                </Button>
              </div>
...
export default EMSPCommandHistoryPage;
