...
const partyFormSchema = z.object({
  ...
  roles: z.string().max(10, "Roles must be 10 characters or less due to database constraints").optional(),
});
...
const PartyForm = ({ initialData, onSuccess }: PartyFormProps) => {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      // Validate roles length to prevent database errors
      if (values.roles && values.roles.length > 10) {
        toast({
          title: "Validation Error",
          description: "Roles field must be 10 characters or less.",
          variant: "destructive"
        });
        return;
      }

      // Format roles - remove spaces around commas for consistency
      const formattedRoles = values.roles?.replace(/\s*,\s*/g, ',') || '';
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input 
                    {...field} 
                    placeholder="CPO,EMSP"
                    maxLength={10}
                  />
                </FormControl>
                <FormDescription>
                  Enter roles as comma-separated values (max 10 characters total).
                  <br />
                  Example: <code>CPO</code> or <code>CPO,EMSP</code>
                </FormDescription>
                <FormMessage />
              </FormItem>
            )}
